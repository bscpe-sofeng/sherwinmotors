...
//            con.Close();
        ...
        private void btnDPdelete_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter the Product's ID number you want to delete");
                txtDProductID.Focus();
                return;
            }            
            if (MessageBox.Show("Are you sure you want to delete this product?", "Message", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                con = new OleDbConnection(constr);
                con.Open();
